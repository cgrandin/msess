...
runAssessments <- function(simDat,
  ...
  for(assessYr in simDat$assessYears){
    ...
    if(nrow(simDat$natage)!=numGenders){
      cat("runAssessment: Warning: The number of genders in file ",natageFile," (",nrow(simDat$natage),"), conflicts \n  with the number of genders in ",
    ...
    # Simulate new commercial fishery age data for year, this changes the data to be written to the assessment datafile
    for(fleet in 1:numFishingFleets){
      simDat <- simulateAgeComps(simDat,assessYr,fleet=fleet)
    }

    # Simulate new survey age data for year, this changes the data to be written to the assessment datafile
    for(survey in 1:numSurveys){
      if(assessYr %in% simDat$survYrs){
        simDat <- simulateSurveyIndex(simDat,assessYr)
        simDat <- simulateAgeComps(simDat,assessYr,fleet=survey,survey=T)
      }
    }
...
simulateAgeComps <- function(simDat,assessYr,fleet,survey=F){
  ...
  # If survey=F then it is a fishery fleet
  ...
  ageComps  <- NULL
  survFleet <- simDat$dat$Nfleet + fleet
  for(sex in 1:length(simDat$sexes)){
    if(survey){
      ageComps            <- rmultinom(1,simDat$survNAge*simDat$survNAgeAdj,probs[sex,])[,1]
      # Here is where the agecomps are inserted with the correct SS format
      simDat$dat$agecomp  <- rbind(simDat$dat$agecomp,c(assessYr,1,survFleet,0,0,.NUM_AGE_ERR_DEFS,-1,-1,simDat$commNAge,ageComps))
    }else{ # Commercial fishery
...
# TODO: proper split-sex structure in the above two insertions
...
simulateSurveyIndex <- function(simDat,assessYear,verbose=TRUE){
  # Calculate the simulated survey index for the given survey 'fleet'.
